...
public class BolasShop : MonoBehaviour
{
    ...
    void FillList()
    {
        foreach (Bolas b in bolasList)
        {
            ...
            {
                item.bolaSprite.sprite = Resources.Load<Sprite>("Sprites/" + b.bolasNomeSprite + "Cinza");
            }
        }
    }
                ...
                for (int j = 0; j < bolasList.Count; j++)
                {
                    if (bolasList[j].bolasID == bola_id)
                    {
                        if (bolasList[j].bolasComprou == true)
                        {
                            ...
                            bolasSuportScript.bolaPreco.text = "Bought!";
                            SalvaBolasLojaInfo(bolasSuportScript.bolaID);
                        }
                        else
                        {
                            bolasSuportScript.bolaSprite.sprite = Resources.Load<Sprite>("Sprites/" + bolasList[j].bolasNomeSprite + "Cinza");
                        }
                    }
                }
            }
